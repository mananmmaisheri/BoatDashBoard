...
This skill allows you to extract visual identity and branding information from any website using Firecrawl's brand extraction format. 
...
2. **Analyze the Raw Data (optional)**: If you need more structure, look inside the `branding` key of the JSON object in `.tmp/`.
    - `colors`: primary, secondary, background, and text colors.
    - `typography` / `fonts`: font families and sizings.
    - `components`: buttons and other UI elements styling.
    - `images`: Logo and open-graph imagery.
3. **Analyze the Markdown Content**: The script also extracts the markdown content and all `images` found on the page. Use this to grab header/hero images or general content that helps with context.
